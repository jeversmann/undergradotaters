						   ...
						   procLocation * where)
{
  ...
  pointerValue & binding = var->get_binding(where);
					   ...
					   pointerValue & return_val)
{
  ...
  pointerValue & ret_bind = ret->get_binding(where);
...
void propertyAnalyzer::at_cast(stmtLocation * current, operandNode *operand,
...
// TB_unify
bool propertyAnalyzer::compare_property_value(Location *where,
                                              memoryBlock *property_block,
                                              procedureInfo *callee) {
  assert(property_block && property_block->property);
  enumPropertyAnn *property = property_block->property;
  enumvalue_set callee_input = property
                               ->lookup_input_to_value(callee, property_block);
  return property->compare_now_value(where, property_block, callee_input);
} // compare_property_value


void propertyAnalyzer::record_input_to_value(procedureInfo *callee,
                                             memoryBlock *property_block,
                                             stmtLocation *callsite) {
  assert(property_block && property_block->property);
  enumPropertyAnn *property = property_block->property;
  enumvalue_set new_values = property->construct_now_value(callsite,
                                                           property_block,
                                                           false);
  property->update_input_to_value(callee, property_block, new_values, callsite);
