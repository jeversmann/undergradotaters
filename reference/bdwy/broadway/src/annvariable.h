...
private:
  ...
  pointerValue _bindings;

  /** @brief Test
   *
   */

  variable_binding_map _all_bindings;
...
public:
  ...
  pointerValue & get_binding() { return _bindings; }
  ...
  void set_binding(memoryBlock * block)
  {
    clear();
    _bindings.blocks.insert(block);
  }

  /** @brief Save bindings */

  void save_bindings(procLocation * where) { _all_bindings[where] = _bindings; }
