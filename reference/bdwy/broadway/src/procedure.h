...
public:
  ...
  callsiteEntry(exprstmtNode * stmt, callNode * call)
    : _stmt(stmt),
      _call(call),
      _contexts()
  {}

  inline exprstmtNode * stmt() const { return _stmt; }
  inline callNode * call() const { return _call; }

  inline proclocation_set & contexts() { return _contexts; }
};

typedef map< exprstmtNode *, callsiteEntry * > callsite_map;
 ...
 * object are properly reflected in assignments. */
  ...
  void add_calling_context(callNode * call, procLocation * where);
  ...
  callsiteEntry * find_callsite(exprstmtNode * stmt);
  ...
  actionAnn * find_applicable_action(exprstmtNode * stmt);
