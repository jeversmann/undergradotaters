			       ...
			       propertyAnalyzer * property_analyzer)
{
  ...
  pointerValue & left = _lhs->binding(where);
  ...
  if (_rhs_is_property_value) {
    ...
    pointerValue & right = _rhs->binding(where);
				     ...
				     memoryblock_set & changes)
{
  ...
  if ( ! _rhs_is_property_value) {
    ...
    _property->apply_next(where, (stmtLocation *)0,
			  ...
			  left, _lhs->name(),
			  right, _rhs_value,
			  _rhs_lost_information, _complicit_property_blocks,
    ...
    _property->apply_next(where, (stmtLocation *)0,
			  ...
			  left, _lhs->name(),
			  right, _rhs_value,
			  false, _complicit_property_blocks,
			  _rhs_ever_values, changes);
  }
}
...
void enumPropertyExprAnn::report(ostream & out, bool is_error, procLocation * where)
{
  ...
  pointerValue & var_binding = _lhs->binding(where);
