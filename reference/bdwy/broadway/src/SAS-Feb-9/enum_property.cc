				       ...
				       bool & lost_information)
{
  ...
  if (_total_order) {
    ...
    enumValueAnn * new_lowest = merge_enumvalue_set(proposed_newval);
  ...
  /*
  if (lost_information) {
    cout << "SET lost information: " << property_block->name() << " at " << * where << endl;
    cout << "   was: " << to_string(oldval) << "    now: " << to_string(newval) 
	 << "    proposed: " << to_string(proposed_newval) << endl;
  }
  */

  /*
  if (property_block->name() == "(global)::stdout_iohandle.FileState") {
  ...
  for (memoryblock_set_p p = left.blocks.begin();
	...
	if (property_block) {
	  ...
	  if (pointerOptions::Monitor_precision) {
	    ...
	    if ((weak_update_lost_information || lost_information) && ! forced_weak) {
					      ...
					      string & indent)
{
  ...
  memoryDef * def = real_block->current_def();
  if (def) {
