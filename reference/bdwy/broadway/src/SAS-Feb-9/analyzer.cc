			      ...
			      bool & never_returns)
{
  ...
  if (procedure == 0) {
    ...
    Pointers::procedure_call(caller, callsite,
			     call, call_target, target_mb, arguments,
	...
	if (header_formal->type()->is_ellipsis()) {
	  ...
	  while (args_p != arguments.end()) {
	    ...
	    for (memoryblock_set_p qq = actual.blocks.begin();
		 ...
		 ++qq)
	      temp.blocks.insert(*qq);

	    args_p++;
	  }

	  formal->binding(libproc_location) = temp;

	  if (pointerOptions::Verbose) {
	    cout << "Ellipsis binding of " << formal->name();
	    print_memoryblock_set(" : ", temp.blocks, cout);
	  }
	  ...
	  if (is_va_list(header_formal)) {
	    ...
	    star_operator(caller, callsite, va_list_actual,
			  local_defs, local_uses, local_changes, actual);

	    // -- Save the bindings:

	    formal->binding(libproc_location) = actual;
	    ...
	    if (args_p == arguments.end()) {
	      cout << "ERROR: Too few arguments to " << target_mb->name() << 
		" at " << * callsite << endl;
	      break;
	    }
  ...
  pointerValue return_ptr(return_block);
  return_var->binding(libproc_location) = return_ptr;
  ...
  process_on_entry(info, libproc_location, procedure,
				...
				memoryblock_set & changes)
{
  ...
  for (structure_list_cp p = on_entry.begin();
	...
	cout << "-- Process on_entry -- " << *s << endl;
      ...
      pointerValue & source = source_var->binding(where);
	...
	{
	  memoryBlock * block = *q;
	  generate_one_use(info, def_use_location, true, block, unused_uses);
	}
      ...
      pointerValue & target = target_var->binding(where);
	...
	dot_operator(def_use_location, s->field_name(), 0, source, uses, target);
      }
				       ...
				       memoryblock_set & changes)
{
  ...
  for (var_map_cp p = variables.begin();
	...
	pointerValue & ptrs = var->binding(where);
	   ...
	   ++p)
	{
	  ...
	  if (target_var->is_new()) {
	    ...
	    if (s->field_name().empty()) {
	      ...
	      target_var->create_binding(new_binding, where);
	    }
	    else {
		...
		*/
		...
		pointerValue after_dot;
		memoryuse_set uses;
		dot_operator(alloc_location, s->field_name(), 0, source, uses, after_dot);

		// -- Retrieve the result. There can only be one because "new"
		// creates a single unique object for each allocation context.

		new_binding = * (after_dot.blocks.begin());
				    ...
				    memoryblock_set & changes)
{
  ...
  // -- When generating reports, there is no need to search for defs

  bool search_for_def = true;
  if (for_reports)
    search_for_def = false;
...
void Analyzer::initialize()
{
  ...
  process_on_entry(info, root_location, init,
		   external_defs, external_uses, external_changes);
