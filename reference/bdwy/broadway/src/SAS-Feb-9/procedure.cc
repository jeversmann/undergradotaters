...
void procedureAnn::add_calling_context(callNode * call, procLocation * where)
{
  exprstmtNode * stmt = (exprstmtNode *) where->stmt_location()->stmt();

  assert(stmt->typ() == Expr);
...
callsiteEntry * procedureAnn::find_callsite(exprstmtNode * stmt)
{
  callsite_map_p p = _callsites.find(stmt);
  if (p != _callsites.end())
    return (*p).second;
...
void procedureAnn::process_reports(Analyzer * analyzer)
{
  ...
  for (callsite_map_cp ccp = callsites().begin();
	   ...
	   ++pp)
	{
	  ...
	  analyzer->create_uses_and_defs((procedureInfo *)0, // -- We shouldn't need this
					 libproc_location,
					 this,
					 true,
					 unused_defs, unused_uses, unused_changes);
...
actionAnn * procedureAnn::find_applicable_action(exprstmtNode * stmt)
{
