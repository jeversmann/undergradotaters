...
Node * patternExpander::at_expr(exprNode * the_expr, Order ord)
{
  ...
  if (the_expr->typ() == Meta) {
    ...
    if (var->name() == "return") {
      ...
      exprstmtNode * exprstmt = (exprstmtNode *) _callsite->stmt();
      exprNode * expr = exprstmt->expr();

      if (expr->typ() == Binary) {
	binaryNode * bin = (binaryNode *) expr;
	if (bin->right()->typ() == Call) {

	  // -- We have a left-hand side, so clone it

	  return ref_clone_changer::clone(bin->left(), false);
	}
      }
    ...
    pointerValue & var_binding = var->binding(where);
...
Node * patternExpander::at_type(typeNode * the_type, Order ord)
{
  ...
  if (the_type->typ() == Meta) {
	...
	if (bin->right()->typ() == Call) {
	  ...
	  if (bin->left()->type())
	    return ref_clone_changer::clone(bin->left()->type(), false);
	}
      }
