			      ...
			      bool & never_returns)
{
  ...
  return_var->set_binding(return_block);
					...
					memoryblock_set & local_changes)
{
  ...
  if (f && (! f->is_void_args())) {
	...
	pointerValue & formal_binding = formal->fresh_binding();
				...
				memoryblock_set & changes)
{
  ...
  for (structure_list_cp p = on_entry.begin();
	...
	cout << "-- Process on_entry -- " << *s << endl;
      ...
      pointerValue & source = source_var->get_binding();
	   ...
	   ++q)
	{
      ...
      pointerValue & target = target_var->fresh_binding();
				       ...
				       memoryblock_set & changes)
{
  ...
  for (var_map_cp p = variables.begin();
	...
	pointerValue & ptrs = var->get_binding();
	   ...
	   ++p)
	{
	  ...
	  if (target_var->is_new()) {
	    ...
	    if (s->field_name().empty()) {
	      ...
	      target_var->set_binding(new_binding);
	    }
	    else {
		...
		*/
		...
		pointerValue & after_dot = target_var->fresh_binding();
		memoryuse_set uses;
			       ...
			       memoryblock_set & changes)
{
  ...
  for (structure_list_cp p = on_exit.begin();
	...
	annVariable * target_var = s->target();
	pointerValue & target = target_var->get_binding();
