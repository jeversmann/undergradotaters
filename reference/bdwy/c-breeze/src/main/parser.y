...
expression:                     /* P */ /* 6.3.17 */
        ...
        | ctokMETA_EXPR    { $$ = new metaexprNode($1); }
        ;
...
/*                        */    /* ? */ /* ?.?.? */
initializer: /*P*/
          ctokLEFTBRACE initializer.list ctokRIGHTBRACE       { $$ = $2; $$->coord($1); }
        | ctokLEFTBRACE initializer.list ctokCOMMA ctokRIGHTBRACE   { $$ = $2; $$->coord($1); }
...
struct.or.union.specifier: /*P*/
          struct.or.union ctokLEFTBRACE struct.declaration.list ctokRIGHTBRACE
            { $$ = $1->set_name_fields_and((idNode *)0, $3, $2, $4); }

        | struct.or.union identifier.or.typedef.name
          ctokLEFTBRACE struct.declaration.list ctokRIGHTBRACE
            { $$ = $1->set_name_fields_and($2, $4, $3, $5); }

        | struct.or.union identifier.or.typedef.name
            { $$ = $1->set_name_and($2, $1->coord()); }

        /* EAB: create rules for empty structure declarations */
        | struct.or.union ctokLEFTBRACE ctokRIGHTBRACE
            { 
              if (CBZ::ANSIOnly)
                 CBZ::Warning(1, string("empty structure declaration"));
	      $$ = $1->set_name_fields_and((idNode *)0, (decl_list *)0, $2, $3);
	    }

        | struct.or.union identifier.or.typedef.name ctokLEFTBRACE ctokRIGHTBRACE
            { 
              if (CBZ::ANSIOnly)
                 CBZ::Warning(1, string("empty structure declaration"));
	      $$ = $1->set_name_fields_and($2, (decl_list *)0, $3, $4);
	    }
        ;
...
/*                        */    /* ? */ /* ?.?.? */
enum.specifier: /*P*/
          ctokENUM ctokLEFTBRACE enumerator.list comma.opt ctokRIGHTBRACE
            { $$ = new enumNode((idNode *)0, $3, $1, Coord($2), Coord($5)); }

        | ctokENUM identifier.or.typedef.name ctokLEFTBRACE enumerator.list comma.opt ctokRIGHTBRACE
...
/*
	    ...
	    */
        ...
        | error ctokSEMICOLON
           {  $$ = (stmtNode *) 0; }
        | ctokMETA_STMT    { $$ = new metastmtNode($1); }
/*
        | empty.statement
           {  }
*/
...
ctokLEFT:  ctokLEFTBRACE  { $$ = $1; CBZ::current_unit->enter_scope(); } ;
ctokRIGHT: ctokRIGHTBRACE  { $$ = $1; CBZ::current_unit->exit_scope(); } ;
...
string.literal.list: /*P*/
        ...
        | string.literal.list STRINGliteral
	      ...
	      $$ = new constNode(constant(s3.c_str()), s3.c_str(), $1->coord());
	      delete $1;
	      delete $2;
	    }
        ;
