...
namespace dataflow {
    ...
    template <class T>
    void DCEVisitor<T>::visitInstruction(Instruction& inst) {
        bool found = false;
    ...
    bool DCEPass::runOnFunction(Function& f) {
        ...
        for (auto &block : f) {
            ...
            for (auto inst = block.rbegin(), end = block.rend(); inst != end; ++inst) {
                ...
                if (state.find(&*inst) == state.end() && !inst->isTerminator() && !inst->mayHaveSideEffects()) {
    ...
    static RegisterPass<DCEPass> X("dce-pass", "Remove dead code from a function using faint variable analysis.", true, true);
