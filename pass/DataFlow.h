...
namespace dataflow {
    ...
    public:
        ...
        DataFlowPass(Meet meet, FlowSet initial, FlowSet boundary = FlowSet(), Direction dir=forward):
        ...
        bool run(T& f) {
            ...
            while(!worklist.empty()) {
                ...
                if (direction == forward) {
                    for (auto itr = pred_begin(node), ite = pred_end(node);
                         itr != ite; ++itr) {
                    ...
                    for (auto itr = succ_begin(node), ite = succ_end(node);
                         itr != ite; ++itr) {
                        applyMeet(startStates[node], endStates[*itr], node);
                        Transfer::postMeet(f, *node);
                    }
